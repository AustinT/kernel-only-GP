...
X_train = np.array([[1, 2, 3], [4, 5, 6]]) / 5
y_train = np.array([0.5, 1.0])
X_test = np.array([[3, 2, 1], [6, 5, 4]]) / 5
...
def test_mll_train(outputscale, noise):
    ...
    with gpytorch.settings.fast_computations(False, False, False), torch.no_grad():
        mll_gpytorch = mll_obj(gp(torch.as_tensor(X_train)), torch.as_tensor(y_train)).item() * len(X_train)

    # Make our GP and get its marginal likelihood
    with torch.no_grad():
        k_train_train = gp.covar_module.base_kernel(torch.as_tensor(X_train)).to_dense().numpy()
...
def test_noiseless_predict(outputscale, noise, full_covar):
    ...
    gp.eval()
    with gpytorch.settings.fast_computations(False, False, False), torch.no_grad():
        output = gp(torch.as_tensor(X_test))
    ...
    if not full_covar:
        k_test_test = np.diag(k_test_test)
        covar_true = np.diag(covar_true)
    ...
    # Test: are they close?
    assert np.allclose(mu_true, our_mu)
    assert np.allclose(covar_true, our_covar, rtol=1e-5)
